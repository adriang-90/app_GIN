...
import (
	...
	"github.com/adriang-90/app_GIN/models"
...
func NewRecipeHandler(
	ctx context.Context, collection *mongo.Collection) *RecipesHandler {
...
func (handler *RecipeHandler) UpdateRecipesHandler(c *gin.Context) {
	id := c.Param("id")
	var recipe models.Recipe
	if err := c.ShouldBindJSON(&recipe); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
